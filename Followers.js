...
export default class ListAvatarExample extends Component {
  ...
  componentDidMount(){
    ...
    .then( json =>{
      ...
      data.forEach(follower => {
        ...
        storeState("followers", this.state.followersName).then(() => {
        retrieveItem("followers").then((map) => {
          console.log(map);
        })
      })
    });
  ...
  follow(userName, userId){
    axios.put('https://api.github.com/user/following/'+userName, {}, {
        headers:{
            ...
            renderItem={ ({item}) =>           
              ...
              <ListItem avatar>
                ...
                <Text style = {{paddingRight : 50, }} onPress={() => this.showProfile(item.a)}>{item.a}</Text>
                  <Button style = {{ height: 30 }} onPress={() => this.follow(item.a, item.c)}><Text style={{fontSize:15, }}>Follow</Text></Button>
                </Body>
                <Right />
              </ListItem>
              </List>}
